...
def run_benchmark(client):
    ...
    cursor.executemany(
        "INSERT INTO data (id, indx, vin, county, city, state, postal_code, model_year, make, model, electric_vehicle_type, "
        "clean_alternative_fuel_vehicle_eligibility, electric_range, base_msrp, legislative_district, dol_vehicle_id, "
        "vehicle_location, electric_utility, census_tract) VALUES (NULL, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, "
        "%s, %s, %s, %s, %s, %s)",
        list_to_insert
    )
    cursor.executemany(
...
def process_factory_row(factories_to_insert, row):
    factories_to_insert.append(row)
